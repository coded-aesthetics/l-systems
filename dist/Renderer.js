export class Renderer {
    ...
    setupMouseControls() {
        ...
        this.canvas.addEventListener("wheel", (event) => {
            ...
            this.zoom = Math.max(1.0, Math.min(20.0, this.zoom));
        ...
        this.canvas.addEventListener("mousemove", (event) => {
            if (isDragging) {
                ...
                if (event.altKey) {
                    ...
                    this.manualRotationX = Math.max(-Math.PI / 2 + 0.1, Math.min(Math.PI / 2 - 0.1, this.manualRotationX));
                    this.canvas.style.cursor = "grabbing";
                }
                lastX = event.clientX;
                lastY = event.clientY;
    ...
    updateGeometry(geometry) {
        ...
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(geometry.depths), gl.STATIC_DRAW);
        gl.vertexAttribPointer(this.attributes.depth, 1, gl.FLOAT, false, 0, 0);
        // Update heights
        gl.bindBuffer(gl.ARRAY_BUFFER, this.heightBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(geometry.heights), gl.STATIC_DRAW);
        gl.vertexAttribPointer(this.attributes.height, 1, gl.FLOAT, false, 0, 0);
        // Update indices
    ...
    updateCamera() {
        ...
        this.mat4RotateX(this.viewMatrix, this.viewMatrix, this.manualRotationX - 0.3);
        this.mat4RotateY(this.viewMatrix, this.viewMatrix, this.manualRotationY + this.rotation);
        // Setup model-view matrix
        this.mat4Identity(this.modelViewMatrix);
        this.mat4Multiply(this.modelViewMatrix, this.viewMatrix, this.modelViewMatrix);
        // Setup normal matrix
    ...
    resize() {
        ...
        if (this.canvas.width !== displayWidth ||
            ...
            this.mat4Perspective(this.projectionMatrix, Math.PI / 4, aspect, 0.1, 100.0);
        }
    }
    render(time) {
        const gl = this.gl;
        ...
        gl.useProgram(this.program);
        // Set uniforms
        gl.uniformMatrix4fv(this.uniforms.modelViewMatrix, false, this.modelViewMatrix);
        gl.uniformMatrix4fv(this.uniforms.projectionMatrix, false, this.projectionMatrix);
        gl.uniformMatrix3fv(this.uniforms.normalMatrix, false, this.normalMatrix);
